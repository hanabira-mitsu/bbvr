...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!this.learningActive)
		{
			...
			if (Input.GetAxisRaw("XRI_Right_Trigger") > 0 && !rightTriggerPulled && Time.timeScale != 0f)
			{
            ...
            if (rightTriggerPulled && Input.GetAxisRaw("XRI_Right_Trigger") == 0)
			{
				rightTriggerPulled = false;
			}
			if ((Input.GetButtonDown("XRI_Left_GripButton") && Time.timeScale != 0f))
			{
				this.DecreaseItemSelection();
			}
			else if ((Input.GetButtonDown("XRI_Right_GripButton") && Time.timeScale != 0f))
			{
	...
	private void UpdateItemSelection()
	{
		this.itemSelect.anchoredPosition = new Vector3((float)this.itemSelectOffset[this.itemSelected], 0f, 0f); //Moves the item selector background(the red rectangle)
		this.UpdateItemName();
		Debug.Log("Item Selection:" + Convert.ToString(itemSelected));
	}
	...
	private void UseItem() //i absolutely HATE raycasts omfg
	{
		if (this.item[this.itemSelected] != 0)
		{
			if (this.item[this.itemSelected] == 1) //zest bar
			{
				this.player.stamina = this.player.maxStamina * 2f;
				this.ResetItem();
			} 
			else if (this.item[this.itemSelected] == 2) //swinging door lock
			{
			...
			else if (this.item[this.itemSelected] == 3) //principals keys (those are in this game??)
			{
				Ray ray2 = new Ray(rightController.transform.position, rightController.transform.forward);
			...
			{
				UnityEngine.Object.Instantiate<GameObject>(this.bsodaSpray, this.rightController.transform.position, new Quaternion(this.rightController.transform.rotation.x, this.rightController.transform.rotation.y, this.rightController.transform.rotation.z, this.rightController.transform.rotation.w));
				this.ResetItem();
				this.player.ResetGuilt("drink", 1f);
				this.audioDevice.PlayOneShot(this.aud_Soda);
			}
			else if (this.item[this.itemSelected] == 5) //a SHINY QUARTER!
			{
				Ray ray3 = new Ray(rightController.transform.position, rightController.transform.forward);
			...
			else if (this.item[this.itemSelected] == 6) //tape
			{
				Ray ray4 = new Ray(rightController.transform.position, rightController.transform.forward);
			...
			else if (this.item[this.itemSelected] == 7) //alarm clock
			{
			...
			else if (this.item[this.itemSelected] == 8) //no-squee
			{
				Ray ray5 = new Ray(rightController.transform.position, rightController.transform.forward);
			...
			else if (this.item[this.itemSelected] == 9) //scissors
			{
				Ray ray6 = new Ray(rightController.transform.position, rightController.transform.forward);
